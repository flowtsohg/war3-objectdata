...
export function objectLoader<T extends IDs>(object: T, modifications: Modification[], props: Prop[], specificProps?: {[key: string]: Prop[] }) {
  for (const modification of modifications) {
    ...
    if (!prop) {
      throw Error(`Failed to get an Ability prop: ${object.oldId} ${id}`);
    }
...
export function objectSaver<T extends IDs>(gameObject: T, object: T, baseProps: Prop[], specificProps?: {[key: string]: Prop[] }): Modification[] {
  const modifications = [];

  for (const prop of baseProps) {
    if ((<any>gameObject)[prop.name] !== (<any>object)[prop.name]) {
      modifications.push(tsToWar3(prop.id, prop.type, (<any>object)[prop.name]));
    }
  }
